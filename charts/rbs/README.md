...
| cleanupAfterHandout | bool | `false` | Whether to publish message to Janitor after resource is cleaned up. |
...
| vault.janitorConfigPath | string | `nil` | Janitor related configs in Vault path prefix. Required if vault.enabled. Now those Janitor configs are generated by Terraform and output to Vault. |
